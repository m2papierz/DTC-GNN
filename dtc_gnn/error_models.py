...
class ErrorModelBase:
    ...
    @abc.abstractmethod
    def prob_distribution(self, probability: float):
        pass
    ...
    def generate(
        ...
        if isinstance(shape, tuple):
            pauli_errors = self.rng.choice(
                ...
                p=self.prob_distribution(probability)
            )
...
class DepolarizingErrorModel(ErrorModelBase):
    """
    ...
    """
    def prob_distribution(self, probability):
        p_x = p_y = p_z = probability / 3
        p_i = 1 - sum((p_x, p_y, p_z))
        return p_i, p_x, p_y, p_z


class BitFlipErrorModel(ErrorModelBase):
    """
...
class PhaseFlipErrorModel(ErrorModelBase):
    """
...
class BitPhaseFlipErrorModel(ErrorModelBase):
    """
